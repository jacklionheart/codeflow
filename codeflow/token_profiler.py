...
class TokenProfiler:
    ...
    def format_hierarchical_report(self, common_prefix: str) -> str:
        ...
        paths = sorted(
            ...
            key=lambda x: (len(Path(x).parts), x)
        )
        ...
        for path in paths:
            ...
            lines.append(
                f"{indent}{name:<50} {node.total_tokens:>10} "
                f"{node.pct_of_parent:>9.1f}% {node.pct_of_total:>9.1f}%"
            )
    ...
    def format_type_report(self) -> str:
        ...
        for ext, count in sorted_types:
            ...
            lines.append(
                f"{ext_name:<20} {count:>10} {pct:>9.1f}%"
            )
...
def extract_files_from_context(context: str, raw: bool) -> Dict[str, str]:
    ...
    if raw:
        ...
        # Parse XML format
        pattern = r'<document .*?>\s*<source>(.*?)</source>.*?(?:<document_content>(.*?)</document_content>|<instructions>(.*?)</instructions>).*?</document>'
        matches = re.findall(pattern, context, re.DOTALL)
...
def profile_code_context(paths: List[Union[str, Path]], raw: bool = False, 
                   extensions: Optional[Tuple[str, ...]] = None) -> Tuple[str, Dict]:
    """
    ...
    report = "\n".join(summary) + "\n\n" + "\n\n".join([
        profiler.format_hierarchical_report(common_prefix),
        profiler.format_type_report()
    ])
...
def generate_flamegraph(data: Dict[str, Any], output_path: str, title: str = "Flame Graph", 
                       width: int = 1200) -> None:
    """
    ...
    logger = logging.getLogger("codeflow.token_profiler")
...
def _generate_flamegraph_html(data_dict: Dict[str, Any], title: str = "Flame Graph", 
                            width: int = 1200) -> str:
    """
    ...
    # Calculate height based on the data depth
